...
  // PIN, CHOICE, CHALLENGEのいずれのモードでも、推測ピン（または選択都市）を表示
...
    // 選択モードで、間違った都市を選択した場合のみ、推測都市を表示
...
    }
  }
   // チャレンジモードでは、中間結果（isChallengeIntermediateがtrue）でもピンを表示
...
      {/* 最終結果画面 (isChallengeIntermediate が false の場合) */}
...
            </div>
            {/* ユーザーの雨温図の表示：データがあり、かつ、PIN/CHALLENGEモードか、CHOICEモードで不正解だった場合 */}
...
      )}
      
      {/* ★★★ 修正箇所：チャレンジモードの中間結果画面 (isChallengeIntermediate が true の場合) ★★★ */}
...
              <Climograph data={targetChartData} />
            </div>
            {/* ユーザーの雨温図の表示: userChartDataの存在チェックを追加し、正しく閉じる */}
            {userChartData && (
              <div>
                <h3 className="text-xl font-semibold text-center text-gray-700 mb-2">
                  Your Pinned Location's Climograph
                </h3>
                <Climograph data={userChartData} />
              </div>
            )}
